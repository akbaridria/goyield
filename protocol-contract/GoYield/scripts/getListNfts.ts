...
export async function getListNft(senderAddr: string) {
  ...
  atc.addMethodCall({
    appID: nftAppId,
    method: nft.appClient.getABIMethod('arc72_custom_getAllTokenIdsByAddr') as algosdk.ABIMethod,
    methodArgs: [senderAddr],
    sender: accountTest.addr,
    signer: algosdk.makeBasicAccountTransactionSigner(accountTest),
    suggestedParams: await algodClient.getTransactionParams().do(),
    boxes: [{ appIndex: nftAppId, name: algosdk.decodeAddress(senderAddr).publicKey }]
  })

  const res = await atc.simulate(algodClient);
  if(!!res.methodResults[0].returnValue) {
    const result = (res.methodResults[0].returnValue as BigInt[]).map((item) => Number(item))
    return result
  } 
  return [];
}